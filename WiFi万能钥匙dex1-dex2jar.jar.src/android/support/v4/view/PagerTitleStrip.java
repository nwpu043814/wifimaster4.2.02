  ...
  implements ViewPager.Decor
{
  ...
  protected void onMeasure(int paramInt1, int paramInt2)
  {
    ...
    int n = View.MeasureSpec.makeMeasureSpec(paramInt2 - m, Integer.MIN_VALUE);
    this.mPrevText.measure(k, n);
    this.mCurrText.measure(k, n);
    this.mNextText.measure(k, n);
    if (i == 1073741824) {
    ...
    {
      return;
      setMeasuredDimension(paramInt1, Math.max(j, this.mCurrText.getMeasuredHeight() + m));
    }
  }
  ...
  void updateTextPositions(int paramInt, float paramFloat, boolean paramBoolean)
  {
    ...
    if (paramInt != this.mLastKnownCurrentPage)
    {
      ...
      i8 = Math.max(Math.max(i9 + i7, i10 + i6), i8 + i5);
    ...
    for (;;)
    {
      ...
      i = Math.min(i2, i4 - this.mScaledTextSpacing - m);
      this.mPrevText.layout(i, j, m + i, this.mPrevText.getMeasuredHeight() + j);
      i = Math.max(n - i1 - k, this.mScaledTextSpacing + i3);
...
/* Location:              /Users/hanlian/Downloads/WiFi万能钥匙dex1-dex2jar.jar!/android/support/v4/view/PagerTitleStrip.class
